  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-mono tracking-wider text-success animate-pulse-glow">
          ● EN COURS
        </span>
      </div>
      ...
      <GlassCard elevated className="!p-0 overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-border/50">
          <div>
            <p className="text-sm font-bold">{profile?.trading_session || 'Session'}</p>
            <p className="text-[10px] text-muted-foreground">{profile?.strategy || 'Strategy'}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-muted-foreground uppercase">Score moy.</p>
            <p className="text-lg font-bold text-orange">{avgScore}%</p>
          </div>
        </div>
      ...
      {todayTrades.length > 0 && (
        <GlassCard className="!p-0">
          <div className="flex items-center justify-between p-4 pb-2">
            <p className="text-xs font-semibold">Trades de la session</p>
            ...
            {todayTrades.map(t => (
              <div key={t.id} className="flex items-center justify-between p-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      {!t.respected_plan && (
                        <span className="text-[9px] font-bold text-destructive bg-destructive/10 px-1.5 rounded">
                          HORS PLAN
                        </span>
                      )}
                    </div>
                ...
                <p className={`text-sm font-bold ${
                  (Number(t.result_amount) || 0) >= 0 ? 'text-success' : 'text-destructive'
                }`}>
      ...
      <div className="space-y-3 pt-2">
        <button
          ...
          className="glow-button w-full py-4 rounded-2xl font-bold text-sm text-primary-foreground flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          <Crosshair className="w-4 h-4" />
          {todayTrades.length >= maxTrades ? 'Limite atteinte' : 'Prendre un trade · Checklist → Détails'}
        </button>
        <button
          onClick={onEndSession}
          className="w-full py-3 rounded-xl text-sm font-medium text-muted-foreground glass-card flex items-center justify-center gap-2 hover:text-destructive transition-colors"
        >
          <X className="w-4 h-4" />
          Terminer la session
        </button>
